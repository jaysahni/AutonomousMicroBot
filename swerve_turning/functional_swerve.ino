...
double position_error_L;
double position_error_R;
double angle_error;
// dillon new
double total_fwd = 0;
double delayer_amt;
...
char movement[200] = "L F10 L F10 L F10 L";
...
void curveTurn(bool dir){
    double radius = 25.0;  // Turn radius in cm
    double outer_distance = (radius + BOT_RADIUS) * 3.14159 * 0.5;  // Quarter circle
    double inner_distance = (radius - BOT_RADIUS) * 3.14159 * 0.5;
    double ratio = (radius - BOT_RADIUS) / (radius + BOT_RADIUS);
    
    reset();
    update();
    
    double left_pwm, right_pwm;
    double left_distance_target, right_distance_target;
    double left_vel_target, right_vel_target;
    
    // Set initial PWM and targets based on direction
    if(dir) {  // Left turn
        left_pwm = 50 * ratio;      // Inner wheel slower
        right_pwm = 50;              // Outer wheel normal
        left_distance_target = inner_distance;
        right_distance_target = outer_distance;
        left_vel_target = velocity_setpoint * ratio;
        right_vel_target = velocity_setpoint;
    } else {  // Right turn
        left_pwm = 50;               // Outer wheel normal
        right_pwm = 50 * ratio;      // Inner wheel slower
        left_distance_target = outer_distance;
        right_distance_target = inner_distance;
        left_vel_target = velocity_setpoint;
        right_vel_target = velocity_setpoint * ratio;
    }
    
    double t0 = millis();
    double target_angular_velocity = velocity_setpoint / radius;  // rad/s
    
    while (true) {
      double elapsed_time = (millis() - t0) / 1000.0;  // Convert to seconds
      update();
      
      // Position control
      position_error_L = dL() - (left_vel_target * elapsed_time);
      position_error_R = dR() - (right_vel_target * elapsed_time);
      
      // Velocity control
      velocity_error_L = vL() - left_vel_target;
      velocity_error_R = vR() - right_vel_target;
      
      // Angular velocity control (without fabs!)
      double actual_angular_velocity = turnRate * 0.07 * PI /180.0;  // Assuming this gives signed value
      double expected_angular_velocity = dir ? target_angular_velocity : -target_angular_velocity;
      angle_error = actual_angular_velocity - expected_angular_velocity;
      
      // Apply corrections
      left_pwm -= (swerve_kP * position_error_L + swerve_kD * velocity_error_L);
      right_pwm -= (swerve_kP * position_error_R + swerve_kD * velocity_error_R);
      
      // Apply angular correction to both wheels oppositely
      left_pwm += swerve_kA * angle_error * 0.5;
      right_pwm -= swerve_kA * angle_error * 0.5;
      
      // Constrain PWM
      left_pwm = constrain(left_pwm, str_min, 400);
      right_pwm = constrain(right_pwm, str_min, 400);
        if (dL() >= left_distance_target *1.5)
      {
        left_pwm = 0;
      }
      if (dR() >= right_distance_target *1.2)
      {
        right_pwm = 0;
      }
      // Check completion
      if (dL() >= left_distance_target && dR() >= right_distance_target) {
        break;
      }
      motors.setSpeeds(left_pwm, right_pwm);
    }
    ...
    motors.setSpeeds(0, 0);
    delay(50 + delayer_amt);
    reset();
}
/* 
//true is left, false is right
void curveTurn(bool dir){
  double distance = (25.0 - BOT_RADIUS)* 3.14159 * 0.5;
  double ratio = (25.0-BOT_RADIUS)/(25.0+BOT_RADIUS);
  reset();
  update();
  double left_pwm = str_min;
  double right_pwm = str_min;
  double t0 = millis(); // Start time in microseconds
  ...
  double velocity_setpoint = 15; // Initialize the velocity setpoint
  double elapsed_time;

  // Main control loop
  while (true)
  {
    elapsed_time = millis() - t0; // Elapsed time in microseconds
    update();
    if(dir){
      position_error_L = dL() - (velocity_setpoint * elapsed_time/1e3);
      position_error_R = dR() - (velocity_setpoint*1/ratio * elapsed_time/1e3);
      velocity_error_L = vL() - velocity_setpoint;
      velocity_error_R = vR() - 1.0/ratio * velocity_setpoint;
      angle_error =  turnRate * 0.07 * (PI/180.0) - velocity_setpoint/(25.0-BOT_RADIUS); 
      left_pwm -= (swerve_kP * position_error_L + swerve_kD * velocity_error_L - swerve_kA * angle_error);
      right_pwm -= (swerve_kP * position_error_R + swerve_kD * velocity_error_R + swerve_kA * angle_error);
    }else{
      position_error_L = dL() - (velocity_setpoint*1/ratio * elapsed_time/1e3);
      position_error_R = dR() - (velocity_setpoint * elapsed_time/1e3);
      velocity_error_L = vL() - 1.0/ratio * velocity_setpoint;
...
void left()
{
  ...
  update();
  delay(200);
  double left_pwm, right_pwm = 100;
  double velocity_error_R;
  while (dR() < BOT_RADIUS * 3.14159*1.10)
  {
    update();
    velocity_error_R = vR() - velocity_setpoint;
