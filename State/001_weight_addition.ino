...
void pivotLeft()
{
  ...
  double right_pwm = 50;
  double velocity_error_R;
  while (ang() < left_ang) /*dR() < BOT_RADIUS * 3.14159*left_slip_factor*/
  {
    update();
    velocity_error_R = vR() *left_slip_factor - velocity_setpoint;
    right_pwm -= swerve_kD * velocity_error_R;
    motors.setSpeeds(-10, right_pwm);
  }
  motors.setSpeeds(-10,-10);
  // delay(turnTime > (millis() - starting) ? turnTime - (millis() - starting) + 150: 150);
  delay(100);
...
void pivotRight()
{
  ...
  double left_pwm = 100;
  double velocity_error_L;
  while (ang() > -right_ang /*dL() < BOT_RADIUS * 3.14159*right_slip_factor*/)
  {
    update();
    velocity_error_L = vL()*right_slip_factor - velocity_setpoint;
    left_pwm -= swerve_kD * velocity_error_L;
    motors.setSpeeds(left_pwm,-10);
  }
  motors.setSpeeds(-10,-10);
  // delay(turnTime > (millis() - starting) ? turnTime - (millis() - starting) + 150: 150);
  delay(100);
