  ...
  void fwd(double distance)
  {
    ...
    while (true)
    {
      ...
      right_pwm = constrain(right_pwm, str_min +1, 400);
      right_pwm -= kPs * ang();
      // Set motor speeds
  ...
  void longf(double distance)
  {
    ...
    double velocity_setpoint = 0; // Initialize the velAocity setpoint
    double elapsed_time;
    ...
    while (true)
    {
      ...
      {
        // Deceleration phase
        double t_dec = elapsed_time - 14 * delta_T_us / 16;
        velocity_setpoint = (256.0 * distance) / (14.0 * delta_T * delta_T) * ((delta_T / 16) - t_dec / 1e6);
      }
      if (velocity_setpoint < 30){
        velocity_setpoint = 30;
      }
      ...
      // Constrain PWM values to valid range
      left_pwm = constrain(left_pwm, str_min, 400);
      right_pwm = constrain(right_pwm, str_min + 3, 400);
      right_pwm -= kPs * ang();
      // Set motor speeds
      motors.setSpeeds(left_pwm, right_pwm);
    }
